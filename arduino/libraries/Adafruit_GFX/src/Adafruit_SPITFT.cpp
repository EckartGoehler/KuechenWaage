...
void Adafruit_SPITFT::writePixelHLine(int16_t x, int16_t y, uint16_t *colors, uint32_t len)
{
  setAddrWindow(x,y,len,1);
  writePixels(colors,len);
}

void Adafruit_SPITFT::writePixelVLine(int16_t x, int16_t y, uint16_t *colors, uint32_t len)
{
  setAddrWindow(x,y,1,len);
  writePixels(colors,len);
}
